...
const schema = { "@context": "https://schema.org", "@type": "CollectionPage", name: "BuildMeasure Material Estimating Guides", url: absoluteUrl("/guides"), description: "A library of practical construction and DIY material estimating guides.", hasPart: guides.map((guide) => ({ "@type": "Article", name: guide[1], url: absoluteUrl(guide[3]) })) };

export default function GuidesPage() {
  return <>
    <SiteHeader ctaHref="/#calculators" ctaLabel="Open calculators" />
    <main className="guide-page"><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}/><section className="guide-hero"><div className="shell guide-hero-grid"><div><nav className="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span aria-hidden="true">/</span><span>Guides</span></nav><p className="eyebrow">BuildMeasure field notes</p><h1>Material estimating guides</h1><p>Learn what to measure, which assumptions change the result, where rounding belongs, and what the calculator cannot decide for your project.</p></div><aside><strong>Use the library in order</strong><ol><li>Read the measurement guide.</li><li>Verify product-label values.</li><li>Run the matching calculator.</li><li>Check plans and supplier rules.</li></ol></aside></div></section><section className="section shell"><div className="section-heading"><div><p className="eyebrow">Eight focused guides</p><h2>Choose the material you are planning.</h2></div><p>Every guide links to a tested calculator and states its scope limits.</p></div><div className="resource-grid">{guides.map(([topic, title, description, href]) => <a href={href} key={href}><span>{topic}</span><h3>{title}</h3><p>{description}</p><strong>Read the guide <ArrowIcon /></strong></a>)}</div></section><section className="standards-section"><div className="shell standards-grid"><div><p className="eyebrow eyebrow-light">Before the first measurement</p><h2>Good estimates preserve uncertainty.</h2><p className="standards-lede">BuildMeasure separates geometry, user-controlled allowances, product-label inputs, and whole-package rounding. It does not hide regional product differences behind a universal default.</p></div><div><a className="button button-primary" href="/guides/material-estimating-basics">Read the estimating workflow <ArrowIcon /></a></div></div></section></main>
    <SiteFooter />
  </>;
}
