...
async function workerResponse(request) {
  ...
  return worker.fetch(fetchRequest, env, { waitUntil() {}, passThroughOnException() {} });
}

function isCompressible(contentType) {
  const normalized = (contentType ?? "").toLowerCase();
  return normalized.startsWith("text/") || normalized.includes("javascript") || normalized.includes("json") || normalized.includes("xml") || normalized.includes("svg");
}
