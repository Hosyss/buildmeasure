...
JobsiteQuant may add an optional material-cost layer after a calculator has
...
- Currency is a user-controlled display label only, such as `$`, `EUR`, or
  `EGP`; JobsiteQuant does not interpret or convert currencies.
...
summary string. No price is uploaded to a JobsiteQuant account and no account is
required.
