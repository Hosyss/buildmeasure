...
const BRICK_URL = `https://jobsitequant.pages.dev${BRICK_PATH}`;
...
test("discovers Brick Calculator from public product surfaces", async () => {
  ...
  assert.match(
    sitemap,
    /<loc>https:\/\/jobsitequant\.pages\.dev\/brick-calculator<\/loc>/,
  );
  ...
  assert.match(llmsSource, /\[Brick Calculator\]\(https:\/\/jobsitequant\.pages\.dev\/brick-calculator\)/);
