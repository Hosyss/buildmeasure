...
const GUIDE_URL = `https://jobsitequant.pages.dev${GUIDE_PATH}`;
...
test("discovers the Brick wall guide from the calculator, homepage, sitemap, and llms.txt", async () => {
  ...
  assert.match(
    sitemap,
    /<loc>https:\/\/jobsitequant\.pages\.dev\/guides\/how-many-bricks-do-i-need<\/loc>/,
  );

  const llmsSource = await readFile(new URL("../public/llms.txt", import.meta.url), "utf8");
  assert.match(
    llmsSource,
    /\[How Many Bricks Do I Need for a Wall\?\]\(https:\/\/jobsitequant\.pages\.dev\/guides\/how-many-bricks-do-i-need\)/,
  );
});
