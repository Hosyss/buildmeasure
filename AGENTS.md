# BuildNumbers agent instructions

This repository is the durable source of truth for the existing BuildNumbers
site. Preserve `.openai/hosting.json` and the existing Sites identity with slug
`buildmeasure`. The approved Cloudflare rebrand uses `buildnumbers.pages.dev`;
...
- Keep production canonical URLs on
  `https://buildnumbers.pages.dev` unless a separately verified
